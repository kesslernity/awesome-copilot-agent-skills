...
One skill per folder under `skills/<category>/<skill-name>/`, with a required `SKILL.md` and optional `references/*.md`. Run `python3 tools/check_skills.py` before opening a pull request; it asserts every published limit for custom skills in declarative agents and rebuilds the zips.
...
- Capability-neutral: the agent reads what is attached, pasted or in its configured knowledge sources; the skill proposes every write, send, move or delete for the user to perform; outputs are complete Markdown in the chat, with one line offering a downloadable file if the agent has that capability.
...
The full rewrite rules that produced the first 45 skills are in `SPEC.md`. Licence: CC BY-SA 4.0, the same as the sibling repositories; a contribution is a contribution under that licence.
