...
Category: `writing-communication` · Skill name: `weekly-status-update-writer` · Upload package: `dist/zips/weekly-status-update-writer.zip`
...
2. **Configure**, expand **Skills**, **Add**, upload `dist/zips/weekly-status-update-writer.zip` (the whole zip, never `SKILL.md` alone). Up to eight skills per agent.
