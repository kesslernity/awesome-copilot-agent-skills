...
Applies a standing change-safety guardrail: lists every create, modify, move, rename, archive or delete before it is proposed, holds destructive items for typed approval, versions file names instead of overwriting, refuses bulk destructive operations and keeps an append-only change journal. Use when the user asks to "clean up this folder", "delete the old versions", "reorganise these files", "archive everything older than a year", "rename and move these documents" or "make sure nothing gets overwritten while we work", or when any task is about to create or modify files, emails or calendar items. Do not use for backing up, journalling or restoring a skills folder, use skills-backup-keeper instead. Drafts for human review; never approves, authorises or signs off.

Category: `meta-skills` · Skill name: `no-delete-guardrail` · Upload package: `dist/zips/no-delete-guardrail.zip`
...
2. **Configure**, expand **Skills**, **Add**, upload `dist/zips/no-delete-guardrail.zip` (the whole zip, never `SKILL.md` alone). Up to eight skills per agent.
