...
Category: `reporting-analysis` · Skill name: `news-monitor-digest` · Upload package: `dist/zips/news-monitor-digest.zip`
...
2. **Configure**, expand **Skills**, **Add**, upload `dist/zips/news-monitor-digest.zip` (the whole zip, never `SKILL.md` alone). Up to eight skills per agent.
